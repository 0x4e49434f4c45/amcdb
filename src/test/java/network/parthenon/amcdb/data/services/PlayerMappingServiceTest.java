...
class PlayerMappingServiceTest {
    ...
    public static void setupDatabase() throws SQLException {
        dbConnection = DriverManager.getConnection("jdbc:sqlite:" + DATABASE_LOCATION);
    }
    @BeforeEach
    public void setup(){
    ...
    public void testMappingConfirmation() {
        ...
        String confCode = playerMappingService.createUnconfirmed(uuid, rng.nextLong());
        assertFalse(playerMappingService.getByMinecraftUuid(uuid, false).isConfirmed(),
                "Player mapping is created in unconfirmed state");
        playerMappingService.confirm(uuid, confCode);
        assertTrue(playerMappingService.getByMinecraftUuid(uuid, true).isConfirmed(),
    ...
    public void testMappingUpdate() {
        ...
        String confCode = playerMappingService.createUnconfirmed(uuid, oldSnowflake);
        assertEquals(oldSnowflake, playerMappingService.getByMinecraftUuid(uuid, false).getDiscordSnowflake());
        playerMappingService.confirm(uuid, confCode);
        playerMappingService.createUnconfirmed(uuid, newSnowflake);
        PlayerMapping newMapping = playerMappingService.getByMinecraftUuid(uuid, false);
